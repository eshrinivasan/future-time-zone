...
export class DisplayComponent implements OnInit {
  ...
  //choose city using select dropdown and update the time for that zone by passing date to date-time plugin
  changeCity(e) {
    this.cityName.setValue(e.target.value, {
      onlySelf: true
    });
    this.selectedCity = this.cityName.value.split(" ")[1];
    this.zone = new Date().toLocaleString(
      "en-US",
      this._merge(this.options, { timeZone: this.selectedCity })
    );
